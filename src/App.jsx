...
function generateFixtures(groups) {
  ...
  groups.forEach((group) => {
    ...
    allRounds.forEach((roundPairings, roundIndex) => {
      roundPairings.forEach(([home, away]) => fixtures.push({
...
function isArchived(tournament) { return String(tournament?.status || '').toLowerCase() === 'archived'; }
function sortTournaments(items) {
  return [...items].sort((a, b) => Number(isArchived(a)) - Number(isArchived(b)) || new Date(b.created_at || 0) - new Date(a.created_at || 0));
}
...
export default function App() {
  ...
  function buildPreview(entries) {
    ...
    const sorted = [...entries]
      .sort((a, b) => Number(b.rating || 0) - Number(a.rating || 0) || String(a.team_name).localeCompare(String(b.team_name)))
  ...
  async function deleteArchivedTournaments() {
    if (!canUseDatabase) return;
    const archived = tournaments.filter(isArchived);
    if (!archived.length) return setStatus('No archived tournaments to delete.');
    if (!window.confirm(`Delete ${archived.length} archived tournament(s) and their fixtures, groups, entries and honours? This cannot be undone.`)) return;
    setLoading(true);
    setStatus('Deleting archived tournaments...');
    try {
      ...
      const { data: archivedMatches, error: matchFindError } = await supabase.from('matches').select('id').in('tournament_id', ids);
      if (matchFindError) throw matchFindError;
      const matchIds = (archivedMatches || []).map((match) => match.id);
      if (matchIds.length) {
      ...
      setPreview(null);
      await loadTournaments();
      setStatus(`Deleted ${archived.length} archived tournament(s).`);
    } catch (error) {
...
function Overview({ tournaments, selectedTournament, setSelectedTournamentId, preview, onDeleteArchived, loading }) {
  const archivedCount = tournaments.filter(isArchived).length;
  return <div className="overview"><div className="overview-actions"><p className="muted">Live and draft tournaments are shown first. Archived tournaments are pushed to the bottom.</p><button type="button" className="danger" onClick={onDeleteArchived} disabled={loading || archivedCount === 0}>Delete archived tournaments ({archivedCount})</button></div><div className="overview-metrics"><article><span>Loaded tournaments</span><strong>{tournaments.length}</strong></article><article><span>Selected</span><strong>{selectedTournament?.name || 'None'}</strong></article><article><span>Preview groups</span><strong>{preview?.groups?.length || 0}</strong></article><article><span>Preview fixtures</span><strong>{preview?.fixtures?.length || 0}</strong></article></div><div className="tournament-grid">{tournaments.map((tournament) => <button key={tournament.id} type="button" className={selectedTournament?.id === tournament.id ? 'tournament-card selected' : 'tournament-card'} onClick={() => setSelectedTournamentId(tournament.id)}><strong>{tournament.name}</strong><span>{tournament.status || 'draft'} · {tournament.actual_entries || 0}/{tournament.max_entries || '?'} entries</span><span>{tournament.group_count || '?'} groups · {tournament.knockout_teams || '?'} knockout teams</span></button>)}</div></div>;
}
