...
if(!window.liveMatchXML&&confirm('Arm replay network trace? After this, open a completed match.')){const trace=document.createElement('script');trace.src=traceScript+'?v='+Date.now();trace.onerror=()=>alert('Could not load replay network trace helper.');(document.head||document.documentElement).appendChild(trace);return;}
