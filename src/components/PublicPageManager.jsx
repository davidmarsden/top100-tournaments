...
function resolveFinal(matches, bracket) {
  ...
  const winnerLeg = ordered.find((match) => match.home_entry_id === winnerId || match.away_entry_id === winnerId);
  const winnerName = winnerLeg ? teamNameFromEntry(winnerLeg.home_entry_id === winnerId ? winnerLeg.home_entry : winnerLeg.away_entry, 'FET/manual winner needed') : 'FET/manual winner needed';
  return { bracket, winnerName, aggregate: `${firstAgg}-${secondAgg}`, needsFet: !winnerId };
}
