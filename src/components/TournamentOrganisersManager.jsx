...
export default function TournamentOrganisersManager({ selectedTournament }) {
  ...
  useEffect(() => { if (targetTournamentId) loadAccess(targetTournamentId); }, [targetTournamentId]);

  const targetTournament = useMemo(
    () => tournaments.find((item) => String(item.id) === String(targetTournamentId)) || selectedTournament || null,
    [tournaments, targetTournamentId, selectedTournament],
  );
  ...
  async function loadAccess(tournamentId) {
  ...
  async function assign(event) {
    ...
    else { setSelectedAccountId(''); await loadAccess(tournamentId); setStatus('Tournament access assigned.'); }
    setLoading(false);
  }
  ...
  async function setActive(row, active) {
    ...
    else { await loadAccess(row.tournament_id); setStatus(active ? 'Tournament access restored.' : 'Tournament access removed.'); }
    setLoading(false);
  }
  ...
  return <div className="registration-manager">
    <section className="entrant-panel"><p className="eyebrow">Tournament-scoped access</p><h3>{targetTournament?.name || 'Choose tournament'}</h3>
      <div className="mini-grid">
        <label>Tournament<select value={targetTournamentId} onChange={(event) => { setTargetTournamentId(event.target.value); setSelectedAccountId(''); }} required><option value="">Choose tournament</option>{tournaments.map((tournament) => <option key={tournament.id} value={tournament.id}>{tournament.name} · {tournament.status || 'draft'}{tournament.registration_status ? ` · registration ${tournament.registration_status}` : ''}</option>)}</select></label>
      </div>
