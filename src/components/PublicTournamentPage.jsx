...
function matchSideClass(match, side) {
  ...
  if (isDoubleForfeit(match)) return 'result-side loser';
...
function buildTables(entries, matches) {
  ...
  return Object.entries(byGroup).sort(([a], [b]) => a.localeCompare(b)).map(([groupCode, groupEntries]) => {
    ...
    matches.filter((match) => match.stage === 'group' && (match.groups?.code || groupCode) === groupCode).filter(isCompleted).forEach((match) => {
      ...
      if (isDoubleForfeit(match)) { home.losses += 1; away.losses += 1; }
...
function publicTournamentPhase(tournament, matches) {
  ...
  if (status === 'draft') return 'Draft · fixtures unpublished';
...
export default function PublicTournamentPage({ tournamentId, routeRows = [] }) {
  ...
  const [knockoutFinalPublicResolved, setKnockoutFinalPublicResolved] = useState(false);

  useEffect(() => { if (hasSupabaseConfig && supabase && tournamentId) loadTournament(); }, [tournamentId]);

  const knockoutOnly = tournament?.tournament_structure === 'knockout_only';
  const datedMatches = useMemo(() => applyRoundDates(matches, roundDates), [matches, roundDates]);
  const knockoutFinalExists = useMemo(() => knockoutOnly && datedMatches.some((match) => match.stage === 'knockout' && match.round === 'Final'), [datedMatches, knockoutOnly]);
  const publicMatches = useMemo(() => {
    if (!knockoutOnly || knockoutFinalPublicResolved) return datedMatches;
    return datedMatches.map((match) => match.stage === 'knockout' && match.round === 'Final'
  ...
  const knockoutBracketOptions = useMemo(() => bracketsFrom(publicMatches), [publicMatches]);
  const knockoutRoundOptions = useMemo(() => roundsFrom(publicMatches.filter((match) => selectedBracket === 'all' || (match.bracket || 'Cup') === selectedBracket)), [publicMatches, selectedBracket]);
  const filteredGroupMatches = useMemo(() => knockoutOnly ? [] : publicMatches.filter((match) => match.stage === 'group' && (selectedGroup === 'all' || (match.groups?.code || 'Ungrouped') === selectedGroup)).sort(groupSort), [publicMatches, selectedGroup, knockoutOnly]);
  const filteredKnockoutMatches = useMemo(() => publicMatches.filter((match) => match.stage === 'knockout' && (selectedBracket === 'all' || (match.bracket || 'Cup') === selectedBracket) && (selectedRound === 'all' || (match.round || 'Round') === selectedRound)).sort(roundSort), [publicMatches, selectedBracket, selectedRound]);
  ...
  const knockoutBrackets = useMemo(() => bracketsFrom(publicMatches), [publicMatches]);
  const tables = useMemo(() => knockoutOnly ? [] : buildTables(entries, publicMatches), [entries, publicMatches, knockoutOnly]);
  ...
  const groupScheduleRows = useMemo(() => knockoutOnly ? [] : groupFixtureSchedule(publicMatches), [publicMatches, knockoutOnly]);
  const nextFixtures = useMemo(() => upcomingMatches(publicMatches), [publicMatches]);
  ...
  const tournamentStatus = String(tournament?.status || 'draft').toLowerCase();
  const fixturesPublished = ['groups_approved', 'published', 'completed', 'archived'].includes(tournamentStatus);
  const unpublishedDraft = tournamentStatus === 'draft';
  const awaitingKnockoutDraw = fixturesPublished && knockoutOnly && !knockoutFinalExists && !knockoutFinalComplete && stats.remaining === 0;
  const remainingDisplay = unpublishedDraft ? 'Not published' : finalReviewPending ? 'Review pending' : awaitingKnockoutDraw ? (stats.fixtures > 0 ? 'Next draw' : 'Draw TBC') : stats.remaining;
  const remainingNote = unpublishedDraft ? 'Fixtures and draw stay hidden until the tournament is published' : finalReviewPending ? 'Final result awaiting confirmation or review' : awaitingKnockoutDraw ? (stats.fixtures > 0 ? 'Current round complete · next round not drawn yet' : 'Opening draw not generated yet') : 'Fixtures still to play';
  ...
  const heroRemaining = unpublishedDraft ? 'fixtures unpublished' : finalReviewPending ? 'Final result under review' : awaitingKnockoutDraw ? (stats.fixtures > 0 ? 'next round pending' : 'draw pending') : `${stats.remaining} fixtures remaining`;
  const heroNextLabel = unpublishedDraft ? 'Not published' : nextFixture ? countdownText(nextFixture) : finalReviewPending ? 'Final result under review' : awaitingKnockoutDraw ? (stats.fixtures > 0 ? 'Next round draw pending' : 'Draw pending') : 'Complete';
  const heroNextDetail = unpublishedDraft ? 'The tournament shell is public, but its draw and fixtures are hidden until the tournament is published.' : nextFixture ? `${formatDate(nextFixture.fixture_date)} · ${fixtureTitle(nextFixture)}` : finalReviewPending ? 'A provisional Final result is awaiting confirmation, admin review or appeal resolution.' : awaitingKnockoutDraw ? (stats.fixtures > 0 ? 'The current round is complete; the next round has not been generated yet.' : 'The knockout draw has not been generated yet.') : 'No upcoming fixtures listed';

  return <main className={`app-shell public-archive tournament-hub${knockoutOnly ? ' knockout-only-hub' : ''}`}>
    ...
    {!knockoutOnly && orderedSeeds.length > 0 && <section id="seedings" className="card"><p className="eyebrow">Draw transparency</p><h2>Rating seedings and pots</h2><div className="standings-wrap"><table className="standings-table seed-table"><thead><tr><th>Seed</th><th>Team</th><th>Manager</th><th>Rating</th><th>Pot</th><th>Group</th></tr></thead><tbody>{orderedSeeds.map((entry) => <tr key={entry.id}><td><strong>{entry.seed || '—'}</strong></td><td><strong>{entry.teams?.name || 'Unknown team'}</strong></td><td>{managerName(entry)}</td><td>{entry.rating ?? '—'}</td><td>{entry.pot ?? '—'}</td><td>{entry.group_code || '—'}</td></tr>)}</tbody></table></div></section>}
    {!knockoutOnly && knockoutBrackets.length > 0 && <section id="brackets" className="card"><p className="eyebrow">Full bracket</p><h2>Cup and Shield bracket</h2><div className="public-bracket-stack">{knockoutBrackets.map((bracket) => <KnockoutBracket key={bracket} title={`${bracket} bracket`} matches={publicMatches.filter((match) => (match.bracket || 'Cup') === bracket)} showChampion />)}</div></section>}
  </main>;
}
