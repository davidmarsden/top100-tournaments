...
export default function PublicTournamentPage({ tournamentId, routeRows = [] }) {
  ...
  return <main className="app-shell public-archive tournament-hub">
    ...
    {knockoutBrackets.length > 0 && <section id="brackets" className="card"><p className="eyebrow">Full bracket</p><h2>{knockoutOnly ? 'Cup bracket' : 'Cup and Shield bracket'}</h2><div className="public-bracket-stack">{knockoutBrackets.map((bracket) => <KnockoutBracket key={bracket} title={`${bracket} bracket`} matches={publicMatches.filter((match) => (match.bracket || 'Cup') === bracket)} showChampion={!knockoutOnly || knockoutFinalPublicResolved} />)}</div></section>}
  </main>;
}
