...
export default function PublicTournamentPage({ tournamentId, routeRows = [] }) {
  ...
  const filteredGroupMatches = useMemo(() => knockoutOnly ? [] : datedMatches.filter((match) => match.stage === 'group' && (selectedGroup === 'all' || (match.groups?.code || 'Ungrouped') === selectedGroup)).sort(groupSort), [datedMatches, selectedGroup, knockoutOnly]);
  ...
  const tables = useMemo(() => knockoutOnly ? [] : buildTables(entries, datedMatches), [entries, datedMatches, knockoutOnly]);
  ...
  const groupScheduleRows = useMemo(() => knockoutOnly ? [] : groupFixtureSchedule(datedMatches), [datedMatches, knockoutOnly]);
  ...
  const knockoutFinalComplete = useMemo(() => {
    if (!knockoutOnly) return false;
    const finals = datedMatches.filter((match) => match.stage === 'knockout' && match.round === 'Final');
    return finals.length > 0 && finals.every((match) => isCompleted(match) && Boolean(match.winner_entry_id));
  }, [datedMatches, knockoutOnly]);
  const awaitingKnockoutDraw = knockoutOnly && !knockoutFinalComplete && stats.remaining === 0;
  const remainingDisplay = awaitingKnockoutDraw ? (stats.fixtures > 0 ? 'Next draw' : 'Draw TBC') : stats.remaining;
  const remainingNote = awaitingKnockoutDraw ? (stats.fixtures > 0 ? 'Current round complete · next round not drawn yet' : 'Opening draw not generated yet') : 'Fixtures still to play';
  ...
  const heroRemaining = awaitingKnockoutDraw ? (stats.fixtures > 0 ? 'next round pending' : 'draw pending') : `${stats.remaining} fixtures remaining`;
  const heroNextLabel = nextFixture ? countdownText(nextFixture) : awaitingKnockoutDraw ? (stats.fixtures > 0 ? 'Next round draw pending' : 'Draw pending') : 'Complete';
  const heroNextDetail = nextFixture ? `${formatDate(nextFixture.fixture_date)} · ${fixtureTitle(nextFixture)}` : awaitingKnockoutDraw ? (stats.fixtures > 0 ? 'The current round is complete; the next round has not been generated yet.' : 'The knockout draw has not been generated yet.') : 'No upcoming fixtures listed';
  ...
  return <main className="app-shell public-archive tournament-hub">
    ...
    <section id="statistics" className="card"><p className="eyebrow">Statistics</p><h2>Tournament numbers</h2><div className="hub-stat-grid"><StatCard label="Completed" value={stats.played} note="Results entered" /><StatCard label="Remaining" value={remainingDisplay} note={remainingNote} />{knockoutOnly ? <StatCard label="Format" value="Knockout" note="Single elimination" /> : <StatCard label="Group winners" value={stats.groupLeaders} note="Current first-place teams" />}<StatCard label="Top scorers" value="Future" note="Ready for player scorers data" /></div></section>
