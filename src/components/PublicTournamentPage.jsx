...
export default function PublicTournamentPage({ tournamentId, routeRows = [] }) {
  ...
  return <main className="app-shell public-archive tournament-hub">
    ...
    <section id="statistics" className="card"><p className="eyebrow">Statistics</p><h2>Tournament numbers</h2><div className="hub-stat-grid"><StatCard label="Completed" value={stats.played} note="Results entered" /><StatCard label="Remaining" value={stats.remaining} note="Fixtures still to play" />{knockoutOnly ? <StatCard label="Format" value="Knockout" note="Single elimination" /> : <StatCard label="Group winners" value={stats.groupLeaders} note="Current first-place teams" />}<StatCard label="Top scorers" value="Future" note="Ready for player scorers data" /></div></section>
