...
function formatShortDate(dateString) { if (!dateString) return ''; const [year, month, day] = String(dateString).slice(0, 10).split('-').map(Number); const date = new Date(Date.UTC(year, month - 1, day)); if (Number.isNaN(date.getTime())) return dateString; return date.toLocaleDateString('en-GB', { day: 'numeric', month: 'short', timeZone: 'UTC' }); }
...
export default function PublicTournamentPage({ tournamentId }) {
  ...
  return <main className="app-shell public-archive">
    ...
    <nav className="public-section-nav" aria-label="Tournament sections"><a href="#format">Format</a><a href="#winners">Winners</a><a href="#groups">Group fixtures</a><a href="#knockout">Knockout</a><a href="#seedings">Seedings</a><a href="#rankings">Cross-group rankings</a><a href="#brackets">Bracket</a></nav>
    <section id="format" className="card format-summary-card"><div className="public-section-toolbar"><div><p className="eyebrow">Tournament format</p><h2>Youth Cup format and rules</h2></div><a className="public-link-button" href={RULES_URL} target="_blank" rel="noreferrer">Read full rules</a></div><div className="format-summary-grid"><article><span>Teams</span><strong>{tournament.actual_entries || entries.length || tournament.max_entries || '—'}</strong><small>{tournament.group_count && tournament.teams_per_group ? `${tournament.group_count} groups of ${tournament.teams_per_group}` : 'Registered entrants'}</small></article><article><span>Group stage</span><strong>{tournament.group_count || '—'}</strong><small>Ranked by points, GD and goals scored</small></article><article><span>Cup</span><strong>{tournament.knockout_teams || '32'}</strong><small>Seeded by group finishing record</small></article><article><span>Shield</span><strong>{tournament.secondary_bracket_name || 'Shield'}</strong><small>Secondary knockout bracket</small></article></div><p className="muted">Teams are seeded by average rating into pots for the group draw. Cross-group tables explain Cup and Shield qualification. Knockout ties show aggregate, away goals and fictional extra time decisions where needed.</p>{groupScheduleRows.length > 0 && <GroupSchedule rows={groupScheduleRows} />}{scheduleRows.length > 0 && <KnockoutSchedule rows={scheduleRows} />}</section>
    <section id="winners" className="card winners-card"><p className="eyebrow">Winners</p><div className="overview-metrics compact-metrics">{winners.length ? winners.map((winner) => <article className="winner-summary-card" key={winner.bracket}><span>🏆 {winner.bracket} winner</span><strong>{winner.winnerName}</strong><small>{winner.firstName} {winner.aggregate} {winner.secondName}{winner.decision ? ` · ${winner.decision}` : ''}</small><div className="mini-results">{winner.legs.map((leg) => <p key={leg.id}>{Number(leg.leg) === 1 ? '1st leg' : '2nd leg'}: {teamName(leg.home_entry, leg.home_placeholder)} {leg.home_score}-{leg.away_score} {teamName(leg.away_entry, leg.away_placeholder)}</p>)}</div></article>) : <p className="muted">No completed finals yet.</p>}</div></section>
