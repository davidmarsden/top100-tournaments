...
export default function PublicTournamentPage({ tournamentId, routeRows = [] }) {
  ...
  return <main className={`app-shell public-archive tournament-hub${knockoutOnly ? ' knockout-only-hub' : ''}`}>
    ...
    {orderedSeeds.length > 0 && <section id="seedings" className="card"><p className="eyebrow">Draw transparency</p><h2>{knockoutOnly ? 'Rating seedings' : 'Rating seedings and pots'}</h2><div className="standings-wrap"><table className="standings-table seed-table"><thead><tr><th>Seed</th><th>Team</th><th>Manager</th><th>Rating</th>{!knockoutOnly && <><th>Pot</th><th>Group</th></>}</tr></thead><tbody>{orderedSeeds.map((entry) => <tr key={entry.id}><td><strong>{entry.seed || '—'}</strong></td><td><strong>{entry.teams?.name || 'Unknown team'}</strong></td><td>{managerName(entry)}</td><td>{entry.rating ?? '—'}</td>{!knockoutOnly && <><td>{entry.pot ?? '—'}</td><td>{entry.group_code || '—'}</td></>}</tr>)}</tbody></table></div></section>}
