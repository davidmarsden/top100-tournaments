...
function forfeitCellClass(value) {
  if (value === null) return 'forfeit-count unavailable';
  if (value >= 3) return 'forfeit-count ineligible';
  return value ? 'forfeit-count' : '';
}
...
export default function TablesManager({ selectedTournament }) {
  ...
  const tables = useMemo(() => buildTables(entries, matches, managerForfeitCounts, !forfeitError), [entries, matches, managerForfeitCounts, forfeitError]);
  ...
  async function loadData() {
    ...
    if (entriesResult.error) setStatus('Could not load entrants: ' + entriesResult.error.message);
      ...
      if (matchRows.length) {
        ...
        if (forfeitsResult.error) disciplineError = 'Could not load manager-forfeit data: ' + forfeitsResult.error.message;
      ...
      setStatus(disciplineError ? 'Tables loaded, but manager-forfeit totals are unavailable.' : 'Tables loaded.');
    }
    setLoading(false);
  }
  ...
  return (
    <div className="tables-manager">
      ...
      {forfeitError && <p className="warning-card"><strong>Manager-forfeit totals could not be loaded.</strong><span>{forfeitError} The Mgr F column is shown as “—” rather than an incorrect zero.</span></p>}
...
function buildTables(entries, matches, managerForfeitCounts, forfeitsAvailable) {
  ...
  return Object.entries(entriesByGroup).sort(([a], [b]) => a.localeCompare(b)).map(([groupCode, groupEntries]) => {
    const rowsById = new Map(groupEntries.map((entry) => [entry.id, blankRow(entry, forfeitsAvailable ? (managerForfeitCounts.get(entry.manager_id) || 0) : null)]));
