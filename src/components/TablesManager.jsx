...
export default function TablesManager({ selectedTournament }) {
  ...
  async function loadData() {
    ...
    if (entriesResult.error) {
      setStatus('Could not load entrants: ' + entriesResult.error.message);
    } else if (matchesResult.error) {
      setStatus('Could not load matches: ' + matchesResult.error.message);
    } else {
    ...
    <div className="tables-manager">
      <div className="fixtures-toolbar">
        <div>
          ...
          <p className="muted">Tables recalculate from saved results. Top two shown as Cup places; lower places shown as Shield places for now.</p>
        </div>
                ...
                <table className="standings-table">
                  ...
                  <tbody>
                    {table.rows.map((row, index) => (
                      <tr key={row.entry_id} className={index < 2 ? 'cup-zone' : 'shield-zone'}>
                        <td>{index + 1}</td>
                        <td>
