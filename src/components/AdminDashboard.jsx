...
const platformModules = ['Overview', 'Organisers', 'Registration', 'Entrants', 'Groups', 'Fixtures', 'Result Approvals', 'Results', 'Tables', 'Forfeits', 'Knockout', 'Reports & Exports', 'Challonge', 'Public Page'];
const organiserModules = ['Overview', 'Registration', 'Entrants', 'Groups', 'Fixtures', 'Results', 'Tables', 'Forfeits', 'Knockout', 'Reports & Exports', 'Public Page'];
...
export default function AdminDashboard() {
  ...
  const { isGlobalAdmin, managedTournamentIds } = useAdminAuth();
  ...
  const modules = isGlobalAdmin ? platformModules : organiserModules;
  ...
  return <main className="app-shell">
    <section className="hero"><div className="hero-row"><div><p className="eyebrow">Top 100 Tournament Manager</p><h1>{isGlobalAdmin ? 'Tournament control centre' : `${scopedTournament?.name || 'Tournament'} organiser`}</h1><p>{isGlobalAdmin ? 'Create tournaments, manage registrations and entrants, generate groups and fixtures, enter results, build knockouts and publish a public tournament page.' : `Your administration access is restricted to ${scopedTournament?.name || 'your assigned tournament'}. Other private tournaments and platform controls are hidden and protected by database permissions.`}</p></div><button type="button" className="secondary admin-logout" onClick={logout}>Log out</button></div></section>
    <ProgressBar selectedTournament={scopedTournament} preview={preview} progressStats={progressStats} onJump={setActiveModule} />
    ...
    <section className="dashboard-layout">
      ...
      <section className="workspace">
        {isGlobalAdmin ? <section className="grid two-columns compact"><TournamentCreateForm onDemoPreview={onDemoPreview} /><WorkflowCard selectedTournament={scopedTournament} preview={preview} progressStats={progressStats} /></section> : <section className="grid two-columns compact"><section className="card"><div className="card-header"><p className="eyebrow">Access scope</p><h2>{scopedTournament?.name}</h2></div><p>Your organiser role applies to this tournament only. Tournament creation, manager accounts, Challonge imports and destructive cross-tournament controls remain with the platform administrator.</p><a className="button secondary" href="/manager">Manager Portal</a></section><WorkflowCard selectedTournament={scopedTournament} preview={preview} progressStats={progressStats} /></section>}
        {canUseDatabase && scopedTournament && <TournamentBuilder selectedTournament={scopedTournament} preview={preview} buildPreview={buildPreview} onNavigate={setActiveModule} onRefresh={refreshTournamentData} />}
        <section className="card module-card"><div className="card-header row"><div><p className="eyebrow">{activeModule}</p><h2>{moduleHeading(activeModule)}</h2></div>{!['Result Approvals', 'Reports & Exports'].includes(activeModule) && <button type="button" className="secondary" onClick={refreshTournamentData} disabled={loading || !canUseDatabase}>Refresh tournament data</button>}</div><ModuleContent activeModule={activeModule} tournaments={manageableTournaments} selectedTournament={scopedTournament} isGlobalAdmin={isGlobalAdmin} /></section>
      </section>
    </section>
  </main>;
}
...
function ModuleContent({ activeModule, tournaments, selectedTournament, isGlobalAdmin }) {
  ...
  if (activeModule === 'Overview') return <Overview tournaments={tournaments} selectedTournament={selectedTournament} setSelectedTournamentId={setSelectedTournamentId} preview={preview} bulkSelectedIds={bulkSelectedIds} setBulkSelectedIds={setBulkSelectedIds} onDeleteTournaments={deleteSelected} onUpdateTournaments={updateTournamentIds} loading={loading} isGlobalAdmin={isGlobalAdmin} />;
...
function Overview({ tournaments, selectedTournament, setSelectedTournamentId, preview, bulkSelectedIds, setBulkSelectedIds, onDeleteTournaments, onUpdateTournaments, loading, isGlobalAdmin }) {
