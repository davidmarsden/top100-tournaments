...
export default function AdminDashboard() {
  ...
  const { selectedTournament, preview, progressStats, canUseDatabase, loading, refreshTournamentData, buildPreview } = useTournament();

  async function logout() { await supabase.auth.signOut(); window.location.href = '/'; }
  function onDemoPreview() { setActiveModule('Groups'); }

  return <main className="app-shell">
    <section className="hero"><div className="hero-row"><div><p className="eyebrow">Top 100 Tournament Manager</p><h1>Tournament control centre</h1><p>Create tournaments, manage registrations and entrants, generate groups and fixtures, enter results, build knockouts and publish a public tournament page.</p></div><button type="button" className="secondary admin-logout" onClick={logout}>Log out</button></div></section>
