...
export default function AdminPollBuilder({ onCreated, setMessage }) {
  ...
  return <section className="card">
    ...
    <form onSubmit={createPoll}>
      ...
      <label>Quorum (%) <span className="muted">— minimum share of eligible managers who must cast a ballot for the poll to be valid</span><input type="number" min="0" max="100" step="0.01" value={quorumPercent} onChange={(event) => setQuorumPercent(event.target.value)} /><span className="muted">For example, a 50% quorum means at least half of the snapshotted electorate must vote. Set 0% for no minimum turnout requirement.</span></label>
