...
function buildStandings(entries, matches) {
  ...
  matches.filter(isPlayed).forEach((match) => {
    const home = rows.get(match.home_entry_id), away = rows.get(match.away_entry_id), homeScore = Number(match.home_score), awayScore = Number(match.away_score);
...
export default function ManagerPortal() {
  ...
  async function submitClaim(event) {
    ...
    const { data: candidateRows, error: candidateError } = await supabase.from('tournament_entries')
      .select('manager_id, managers(id, name, display_name), teams(name)');
    ...
    const matches = (candidateRows || []).filter((row) => {
      const recordedManager = row.managers?.display_name || row.managers?.name;
      return normalise(recordedManager) === normalise(managerName) && normalise(row.teams?.name) === normalise(clubName);
    });
    const managerIds = [...new Set(matches.map((row) => row.manager_id).filter(Boolean))];
    const suggestedManagerId = managerIds.length === 1 ? managerIds[0] : null;

    const payload = {
      auth_user_id: session.user.id,
      email: session.user.email,
      claimed_manager_name: managerName,
      claimed_club_name: clubName,
      suggested_manager_id: suggestedManagerId,
      status: 'pending',
      review_notes: null,
      reviewed_by: null,
      reviewed_at: null,
    ...
    if (error) setMessage('Could not submit your claim: ' + error.message);
    else { setClaim(data); setMessage(suggestedManagerId ? 'Profile found. Your claim is waiting for administrator approval.' : 'Claim submitted. An administrator will match it to the correct manager record.'); }
    setLoading(false);
  }
  ...
  async function loadPortal() {
    ...
    setAccount(accountRow); setClaim(null); setEntries(orderedEntries); setSelectedEntryId(orderedEntries[0]?.id || ''); setMatches(matchRows); setGroupEntries(peerEntries); setMessage('Portal loaded.'); setLoading(false);
  }
  ...
  if (!session) return <main className="manager-portal-shell"><section className="manager-portal-hero"><p className="eyebrow">Top 100 Tournament Manager</p><h1>Manager Portal</h1><p>Your fixtures, results, group table and tournament progress in one place.</p></section><section className="card manager-login-card"><h2>Sign in securely</h2><p className="muted">Enter your email address. We’ll send a one-time sign-in link; new users can then claim their manager profile.</p><form onSubmit={sendMagicLink}><label>Email address<input type="email" value={email} onChange={(event) => setEmail(event.target.value)} required autoComplete="email" /></label><button type="submit" disabled={loading}>{loading ? 'Sending...' : 'Email me a sign-in link'}</button></form>{message && <p className="status">{message}</p>}</section></main>;
  if (loading) return <main className="manager-portal-shell"><section className="card"><h1>Loading Manager Portal...</h1></section></main>;
  if (!account) return <main className="manager-portal-shell"><section className="manager-portal-hero"><div><p className="eyebrow">Manager Portal</p><h1>{claim?.status === 'pending' ? 'Claim awaiting approval' : 'Claim your profile'}</h1><p>Signed in securely as {session.user.email}</p></div><button type="button" className="secondary" onClick={logout}>Sign out</button></section><section className="card manager-login-card">{claim?.status === 'pending' ? <><h2>We’ve got your claim</h2><p><strong>{claim.claimed_manager_name}</strong> · {claim.claimed_club_name}</p><p className="muted">A tournament administrator needs to approve the link. Refresh this page after they confirm it.</p><button type="button" onClick={loadPortal}>Check approval</button></> : <><h2>Match your Top 100 identity</h2><p className="muted">Enter your exact SM manager name and current club as shown in the tournament records. Public information alone does not grant access; an administrator confirms every new claim.</p><form onSubmit={submitClaim}><label>SM manager name<input value={claimForm.managerName} onChange={(event) => setClaimForm((current) => ({ ...current, managerName: event.target.value }))} required /></label><label>Current club<input value={claimForm.clubName} onChange={(event) => setClaimForm((current) => ({ ...current, clubName: event.target.value }))} required /></label><button type="submit">Submit manager claim</button></form>{message && <p className="status">{message}</p>}</>}</section></main>;
