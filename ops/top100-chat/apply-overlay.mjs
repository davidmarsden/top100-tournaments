...
const helperBlock=[
  ...
  '\t\t\treturn;',
  '\t\t\t}',
  '\t\tfunction setDisplayPrefs (userRec) {',
  ...
  '\t\t\tconst sqltext = "update users set prefs = " + davesql.encode (JSON.stringify (prefs)) + " where screenname = " + davesql.encode (userRec.screenname) + ";";',
  ...
  '\t\t\t\t});',
  '\t\t\t}',
  '\t\tgetUserInfoByEmail (cleanEmail, function (err, existingUser) {',
  '\t\t\tif (err) { callback (err); return; }',
  '\t\t\tif (existingUser !== undefined) {',
  '\t\t\t\tsetDisplayPrefs (existingUser);',
  '\t\t\t\treturn;',
  '\t\t\t\t}',
  '\t\t\tconst screenname = "manager" + numericManagerId;',
  '\t\t\tgetUserInfoByScreenname (screenname, function (err, screennameUser) {',
  '\t\t\t\tif (err) { callback (err); return; }',
  '\t\t\t\tif ((screennameUser !== undefined) && (String (screennameUser.emailAddress).toLowerCase () !== cleanEmail)) {',
  '\t\t\t\t\tcallback ({message: "That Top 100 manager id is already linked to another chat account."});',
  '\t\t\t\t\treturn;',
  '\t\t\t\t\t}',
  '\t\t\t\tif (screennameUser !== undefined) { setDisplayPrefs (screennameUser); return; }',
