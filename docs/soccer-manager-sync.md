...
Staging persists only normalized Top 100 data. Raw Soccer Manager responses, cookies, request headers and browser-session credentials are never written to Supabase. Source URLs are sanitized again before persistence and sensitive-looking query parameters are redacted.
...
The browser converts each normalized response into stable source entities before staging. Current entity types include worlds, divisions, standings, manager assignments, fixtures/results, season history, player leaderboards, squad players, transfers, player changes and club finance snapshots. World-specific market events first use the entry's own sanitized source URL for `sid`, then fall back to an unambiguous batch world; if neither establishes a world, the event is not staged as a canonical entity.
