...
const chooseResumeBundle=()=>new Promise(resolve=>{const input=document.createElement('input');let settled=false;const finish=value=>{if(settled)return;settled=true;input.remove();resolve(value);};input.type='file';input.accept='.json,application/json';input.style.display='none';input.oncancel=()=>finish(null);input.onchange=async()=>{try{const file=input.files?.[0];if(!file){finish(null);return;}const data=JSON.parse(await file.text());if(data?.kind!=='worldSeasonMatchBackfill'||!Array.isArray(data.reports))throw new Error('That is not a Top 100 game-world backfill bundle.');finish(data);}catch(err){alert('Could not read resume bundle: '+String(err&&err.message||err));finish(null);}};document.body.appendChild(input);input.click();});
...
const saveBundle=(chunks,manifest,state)=>{const bundle={kind:'worldSeasonMatchBackfill',version:2,runId:manifest.runId,capturedAt:manifest.capturedAt,setupId:manifest.setupId,discovery:manifest.summary,clubs:manifest.clubs,reports:chunks.flat(),failures:manifest.failures,crawlState:state};download(bundle,1);};
...
(async()=>{window.__top100WorldBackfillRunning=true;const progress=makeProgress(),startedAt=Date.now();try{
 ...
 if(resume){
   ...
   const priorFetched=Array.isArray(resume.crawlState?.fetchedFixtureIds)?resume.crawlState.fetchedFixtureIds:[];
   for(const id of priorFetched){const valid=nonZeroId(id);if(valid){fetched.add(valid);queued.add(valid);}}
   ...
   const priorPending=Array.isArray(resume.crawlState?.pendingFixtureIds)?resume.crawlState.pendingFixtureIds:[];
   for(const id of priorPending)enqueue(id);
   ...
   progress.update('<b>Resume loaded:</b> '+fetched.size+' cumulative captured fixtures skipped · '+resumedFailures+' previous failures queued for retry<br><b>Queue:</b> '+queue.length+' remaining');
 }
 ...
 const crawlState={fetchedFixtureIds:[...fetched].filter(id=>!failures.some(f=>f.fixtureId===id)),pendingFixtureIds:[...queue],knownFixtureIds:[...queued]};
 if(ok)saveBundle(chunks,manifest,crawlState);
