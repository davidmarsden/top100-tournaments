...
(async()=>{window.__top100WorldBackfillRunning=true;try{
 ...
 chunks.forEach((reports,index)=>download({kind:'worldSeasonMatchBackfill',version:1,runId,chunk:index+1,chunks:chunks.length,capturedAt:new Date().toISOString(),setupId,discovery:{source:'schedule seed + completed-fixture graph',seedFixtures:seedIds.length,uniqueFixturesSeen:queued.size,reportsAttempted:fetched.size,clubsSeen:clubs.size,queueRemaining:queue.length,safetyCap:MAX_REPORTS},clubs:[...clubs].map(([clubId,name])=>({clubId,name})),reports,failures:index===chunks.length-1?failures:[]},index+1));
 alert('Game-world crawl complete: '+(fetched.size-failures.length)+' reports captured across '+clubs.size+' clubs in '+chunks.length+' import chunk(s).'+(queue.length?' Safety cap reached with '+queue.length+' fixture(s) still queued.':'')+' Import all downloaded chunks in Soccer Manager Sync.');
