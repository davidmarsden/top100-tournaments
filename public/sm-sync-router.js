...
if(!window.liveMatchXML&&confirm('Arm replay network trace? After this, open a completed match.')){load('top100-sm-replay-network-trace-script',origin+'/sm-replay-network-trace.js',()=>alert('Could not load replay network trace helper.'));return;}
const session=(crypto&&crypto.randomUUID?crypto.randomUUID():Date.now().toString(36)+Math.random().toString(36).slice(2));
const win=window.open(origin+'/admin/soccer-manager-sync?collectorSession='+encodeURIComponent(session),'top100-sm-sync');
