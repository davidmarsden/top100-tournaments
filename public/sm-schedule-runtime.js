...
const scripts=[...document.scripts].map(s=>s.src).filter(Boolean).map(raw=>{try{const u=new URL(raw,location.href);return u.origin===location.origin?u.href:null;}catch{return null;}}).filter(Boolean).slice(-MAX_SCRIPTS);
const resources=performance.getEntriesByType('resource').map(e=>{try{const u=new URL(e.name,location.href);if(u.origin!==location.origin)return null;return {url:u.pathname+u.search,initiatorType:e.initiatorType};}catch{return null;}}).filter(Boolean).filter(x=>/(schedule|club|multi|fixture|result)/i.test(x.url)).slice(-160);
