...
const nonZeroId=(v)=>{const id=text(v);return id&&id!=='0'?id:null;};const setupId=nonZeroId(page.searchParams.get('sid'))||nonZeroId(seed?.SetupID??seed?.setupId??seed?.GameWorldID??seed?.gameWorldId);const data={kind:'hamburgSeasonMatchBackfill',version:2,capturedAt:new Date().toISOString(),sourcePage:location.origin+location.pathname,setupId,targetClub:{clubId:TARGET_CLUB_ID,name:TARGET_NAME},summary:{fixturesDiscovered:[...discovered.values()].filter(row=>clubIds(row).includes(TARGET_CLUB_ID)&&played(row)).length,reportsFetched:reports.length,discoveryReportsFetched:fetched.size,nonHamburgDiscoveryReports:nonHamburgReports,failures:failures.length,byCompetition},reports,failures};
