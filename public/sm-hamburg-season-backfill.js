...
(async()=>{window.__top100HamburgBackfillRunning=true;try{
 ...
 for(let i=0;i<completedRows.length;i++){const row=completedRows[i],id=fixtureId(row);try{const raw=await fetchReport(id);const report=normalizeReport(raw);if(report.home.clubId!==TARGET_CLUB_ID&&report.away.clubId!==TARGET_CLUB_ID)throw new Error('Fetched report does not involve Hamburger SV');reports.push(report);}catch(err){failures.push({fixtureId:id,error:String(err&&err.message||err)});}if(i+1<completedRows.length)await sleep(DELAY_MS);}
 ...
 const page=new URL(location.href),setupId=nonZeroId(page.searchParams.get('sid'))||nonZeroId(window.g_setupid??window.g_setupId??window.g_gameworldid);
