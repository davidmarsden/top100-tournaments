...
const normalizeReport=(raw)=>{const side=text(raw?.HomeClubID)===TARGET_CLUB_ID?'home':text(raw?.AwayClubID)===TARGET_CLUB_ID?'away':null;const prefix=side==='home'?'h_':side==='away'?'a_':null;const opponent=side==='home'?text(raw?.AwayTeamName):side==='away'?text(raw?.HomeTeamName):null;const stats=prefix?{possession:number(raw[prefix+'possession']??raw[prefix+'Possession']),shots:number(raw[prefix+'shots']??raw[prefix+'Shots']),shotsOnTarget:number(raw[prefix+'shotsontarget']??raw[prefix+'ShotsOnTarget']),corners:number(raw[prefix+'corners']??raw[prefix+'Corners'])}:null;return {fixtureId:text(raw?.fixtureID??raw?.FixtureId),date:text(raw?.TurnDate??raw?.FullTurnDate),competition:text(raw?.TournName??raw?.TournamentName),home:{clubId:text(raw?.HomeClubID),name:text(raw?.HomeTeamName),score:number(raw?.HomeScore)},away:{clubId:text(raw?.AwayClubID),name:text(raw?.AwayTeamName),score:number(raw?.AwayScore)},hamburg:{side,opponent,stats},schema:{topLevelFields:Object.keys(raw||{}).sort(),fieldCount:Object.keys(raw||{}).length},raw};};
...
try{
const seedUrl=new URL('/matchreport-ajax-mobile.php',location.origin);const page=new URL(location.href);const pageFixture=text(page.searchParams.get('fixtureid')||page.searchParams.get('fixid')||page.searchParams.get('matchid'));
if(!pageFixture){alert('Open one completed Hamburger SV match first, then run Top 100 Sync → Backfill Hamburg season.');return;}
...
const candidates=[...discovered.values()].filter(row=>clubIds(row).includes(TARGET_CLUB_ID)&&played(row)).slice(0,MAX_FIXTURES);
if(!candidates.some(row=>fixtureId(row)===pageFixture))candidates.unshift({FixtureId:pageFixture,HomeTeamId:text(seed?.HomeClubID),AwayTeamId:text(seed?.AwayClubID),Played:'1'});
const unique=[...new Map(candidates.map(row=>[fixtureId(row),row])).values()];
const reports=[],failures=[];
for(let i=0;i<unique.length;i++){const id=fixtureId(unique[i]);try{const u=new URL('/matchreport-ajax-mobile.php',location.origin);u.searchParams.set('fixtureid',id);u.searchParams.set('action','mr');const response=await fetch(u.href,{credentials:'include'});if(!response.ok)throw new Error('HTTP '+response.status);const raw=await parseJson(response);reports.push(normalizeReport(raw));}catch(err){failures.push({fixtureId:id,error:String(err&&err.message||err)});}if(i<unique.length-1)await sleep(DELAY_MS);}
...
const data={kind:'hamburgSeasonMatchBackfill',version:1,capturedAt:new Date().toISOString(),sourcePage:location.origin+location.pathname,targetClub:{clubId:TARGET_CLUB_ID,name:TARGET_NAME},summary:{fixturesDiscovered:unique.length,reportsFetched:reports.length,failures:failures.length,byCompetition},reports,failures};
download(data);alert('Hamburg backfill complete: '+reports.length+'/'+unique.length+' reports fetched'+(failures.length?' ('+failures.length+' failed)':'')+'. JSON downloaded for review.');
