...
window.fetch=async function(input,init){const raw=input instanceof URL?input.href:typeof input==='string'?input:input&&input.url;const url=projectUrl(raw);const method=String((init&&init.method)||(input&&input.method)||'GET').toUpperCase();let fields=bodyFields(init&&init.body);if(!fields.length&&(!init||init.body===undefined)&&input instanceof Request){try{const clone=input.clone();const type=(clone.headers.get('content-type')||'').toLowerCase();if(type.includes('application/x-www-form-urlencoded')){const text=await clone.text();if(text.length<=10000)fields=bodyFields(text);}}catch{}}const start=performance.now();try{const response=await originalFetch.apply(this,arguments);if(url)add({transport:'fetch',method,url,fields,status:response.status,contentType:(response.headers.get('content-type')||'').slice(0,120),duration:Math.round(performance.now()-start)});return response;}catch(err){if(url)add({transport:'fetch',method,url,fields,error:String(err&&err.message||err).slice(0,160),duration:Math.round(performance.now()-start)});throw err;}};
