-- Treat Soccer Manager numeric managed flags as managed when positive.
-- Live Top 100 captures use 2 for most managed clubs and 1 for one managed club.
...
begin
  ...
  loop
    ...
    if coalesce(v_existing_source_name, v_existing_team_name) is not null
      ...
      update public.game_world_clubs
        set club_name = v_club_name,
            ...
            occupied = case when (coalesce(row_data.data->>'managed', '') ~ '^[1-9][0-9]*$' or lower(coalesce(row_data.data->>'managed', '')) in ('true','yes')) then true else false end,
            current_manager_name = case
              when (coalesce(row_data.data->>'managed', '') ~ '^[1-9][0-9]*$' or lower(coalesce(row_data.data->>'managed', '')) in ('true','yes'))
    ...
    insert into public.game_world_clubs (
      ...
      case when (coalesce(row_data.data->>'managed', '') ~ '^[1-9][0-9]*$' or lower(coalesce(row_data.data->>'managed', '')) in ('true','yes')) then true else false end,
  ...
  loop
    ...
    select case
      when (coalesce(standing.data->>'managed', '') ~ '^[1-9][0-9]*$' or lower(coalesce(standing.data->>'managed', '')) in ('true','yes')) then true
      else false
    end
      into v_club_managed
