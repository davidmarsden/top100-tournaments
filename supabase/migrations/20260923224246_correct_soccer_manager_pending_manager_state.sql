-- Correct Soccer Manager current-manager status handling.
-- In live Top 100 competition data, managed=2 is current/occupied and managed=1 is a pending application.
-- Clearing a non-current assignment also deactivates that manager's world membership.
...
begin
  ...
  loop
    ...
    if v_team_id is null then
      ...
      if v_case_matches > 0 then
        raise exception 'Soccer Manager club rename % (%) collides with another Top 100 team', v_club_name, v_club_id;
      end if;
    end if;
    ...
    if coalesce(v_existing_source_name, v_existing_team_name) is not null
      ...
      update public.game_world_clubs
        set club_name = v_club_name,
            ...
            occupied = case when (coalesce(row_data.data->>'managed', '') = '2' or lower(coalesce(row_data.data->>'managed', '')) in ('true','yes')) then true else false end,
            current_manager_name = case
              when (coalesce(row_data.data->>'managed', '') = '2' or lower(coalesce(row_data.data->>'managed', '')) in ('true','yes'))
              then current_manager_name else null end,
            manager_key = case
              when (coalesce(row_data.data->>'managed', '') = '2' or lower(coalesce(row_data.data->>'managed', '')) in ('true','yes'))
    ...
    insert into public.game_world_clubs (
      ...
      case when (coalesce(row_data.data->>'managed', '') = '2' or lower(coalesce(row_data.data->>'managed', '')) in ('true','yes')) then true else false end,
    ...
    on conflict (game_world_id, club_key) do update
      set club_name = excluded.club_name,
          ...
          current_manager_name = case
            when excluded.occupied then public.game_world_clubs.current_manager_name else null end,
          manager_key = case
            when excluded.occupied then public.game_world_clubs.manager_key else null end,
    ...
    insert into public.soccer_manager_archive_links (
      ...
      'club', setup_id || ':' || v_club_id, 'team', v_team_id, v_club_name
    )
    on conflict (source_type, source_key) do update
      set target_type = excluded.target_type,
          target_id = excluded.target_id,
  ...
  -- Stable manager ids are mapped once. The same normalized registration key used
  -- by the manager directory is used only for the first link; ambiguous matches abort.
  for row_data in
  ...
  loop
    ...
    select case
      when (coalesce(standing.data->>'managed', '') = '2' or lower(coalesce(standing.data->>'managed', '')) in ('true','yes')) then true
      else false
    end
      into v_club_managed
    ...
    if coalesce(v_club_managed, false) = false then
      ...
      update public.manager_game_world_memberships membership
        set active = false,
            updated_at = now()
      where membership.game_world_id = v_world_id
        and membership.manager_id in (
          select assignment.manager_id
          from public.soccer_manager_world_manager_assignments assignment
          where assignment.game_world_id = v_world_id
