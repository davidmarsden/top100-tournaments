...
-- The legacy match-status trigger normally turns any match with status=forfeit
-- into a permanent disciplinary/prize-draw record. Manager Portal forfeits are
-- provisional until an admin finalises them, so suppress that permanent write
-- while the related submission is still open.
...
-- If this migration follows an earlier deployment of the provisional workflow,
-- clean up any permanent rows that may have been created before the trigger was
-- made submission-aware.
