...
using (
  ...
  or exists (
    ...
    join public.voting_electorate e on e.manager_id = a.manager_id and e.event_id = ve.id
...
begin
  ...
  if v_event.status not in ('open','closed') then
    raise exception 'Only closed or expired open events can be finalised';
  end if;
  ...
  if v_ballots > 0 and v_tie_count = 1 then
